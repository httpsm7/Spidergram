...
def run_once(dry_run: bool = False) -> None:
    """Run the full pipeline once for all agents (non-blocking)."""
    logger.info(f"Running pipeline once (dry_run={dry_run})...")
    try:
        flush_task_queue()
        _run_all_agents(dry_run=dry_run)
        logger.info("Single run complete.")
    except Exception as exc:
        logger.error(f"run_once error: {exc}")
